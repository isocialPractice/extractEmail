...
This installs dependencies and automatically compiles the TypeScript sources in `src/` to `dist/` via the `prepare` script. To rebuild after editing sources, run:
...
│   ├── extractEmail.ts        # Main CLI entry point
│   ├── cli.ts                 # CLI argument parsing
│   ├── imap.ts                # IMAP connection and fetch
│   ├── parse.ts               # Email parsing helpers
│   ├── output.ts              # Output formatting
│   └── helpers/               # Shared helper modules (TS)
│       ├── dateHelper.ts      # {{ dates.* }} values via @jhauga/getDate
│       ├── filterHelper.ts    # {{ regex }} / {{ dates.* }} filter resolution
│       ├── emailChain.ts      # Email chain detection and segment classification
│       ├── objects.ts         # Extraction result objects and markers
│       └── narrowRequestedData.ts  # Natural-language email task parser
├── dist/                      # Compiled JavaScript (generated by `npm run build`)
├── helpers/                   # Re-export shims pointing at dist/helpers/
├── extractEmail.mjs           # Re-export shim for legacy entry point
├── configEmailExtraction.mjs  # Default IMAP configuration (from template)
├── config.json                # Multi-account folder configuration
├── package.json               # Project metadata and dependencies
├── tsconfig.json              # TypeScript compiler configuration
├── accounts/                  # Account configuration files
│   └── example.mjs            # Example account config (from template)
...
├── config/                    # Map and filter configuration files
│   └── mapRequestedData/      # Document/recipient maps for narrowRequestedData
└── test/                      # Test utilities

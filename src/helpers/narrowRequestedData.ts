...
import { classifyAndNarrowChain } from './emailChain.js';
 ...
 */
function parseEmailTask(messageText, referenceDate = new Date(), options: any = {}) {
  ...
  let recipient: any = extractRecipient(processedText);
...
export function runCli(args = process.argv.slice(2)) {
  ...
  if (userMessage) {
    ...
    const projectRoot = resolve(__dirname, '..', '..'); // dist/helpers/ -> dist/ -> project root

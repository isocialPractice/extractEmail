...
export function splitChain(rawBody) {
  ...
  const boundaries: any[] = [{ lineIndex: 0, headers: {} }];
 ...
 */
export function findBestSegment(segments, opts: any = {}) {
 ...
 * @returns {string}
 */
export function extractBestBody(rawBody, opts: any = {}) {
...
export function classifyChain(segments) {
  ...
  let dominantType: string = SEGMENT_TYPES.UNCLASSIFIED;
  let maxCount = 0;
 ...
 * @returns {string}
 */
export function classifyAndNarrowChain(rawBody, opts: any = {}) {
 ...
 * @returns {string}
 */
export function extractBestBodyByTask(rawBody, bodyPattern, narrowTypeBy = 'date-range', opts: any = {}) {
  const { stripCidLines = true } = opts;

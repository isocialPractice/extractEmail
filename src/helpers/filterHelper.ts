...
import { getDateValues } from './dateHelper.js';
...
export function resolveFilterPattern(pattern) {
  ...
  if (!pattern.includes('{{')) {
    const result: any = { type: 'string', value: pattern };
  ...
  try {
    ...
    const fb: any = { type: 'string', value: plainText };
  ...
  const result: any = { type: 'regex', value: regex };
